    ...
    @SuppressWarnings("unchecked")
        ...
        private void deserializeTransporters(CompoundTag blockEntityList)
        {
                ...
                transportersTag.getAllKeys().stream().forEach(transporterString ->
                {
                        ...
                        if(transporter != null)
                        {
                                if(!transporter.getID().toString().equals(transporterString))
                                        recoveredCount[0]++;
                ...
                if(recoveredCount[0] > 0)
                        StargateJourney.LOGGER.warn("BlockEntityList: recovered {} transporter(s) with malformed UUID keys (new UUIDs assigned)", recoveredCount[0]);
                if(skippedCount[0] > 0)
                        StargateJourney.LOGGER.warn("BlockEntityList: skipped {} transporter entry(ies) that could not be recovered", skippedCount[0]);
