...
public final class TransporterNetwork extends SavedData
{
        ...
        public void updateNetwork(MinecraftServer server)
        {
                ...
                StargateJourney.LOGGER.warn("Transporter Network version migration: {} -> {}. All transporter connections and dimension mappings will be cleared and re-registered.", getVersion(), UPDATE_VERSION);
        ...
        private void deserializeConnections(CompoundTag tag)
        {
                ...
                for(String connectionID : tag.getAllKeys())
                {
                        try
                        {
                                ...
                                TransporterConnection connection = TransporterConnection.deserialize(server, uuid, tag.getCompound(connectionID));
                ...
                if(malformedCount > 0)
                        StargateJourney.LOGGER.warn("TransporterNetwork: skipped {} connection(s) with malformed UUID keys", malformedCount);
        }
    ...
    private static class Dimension
    {
        ...
        public CompoundTag serialize()
                ...
                int[] malformed = {0};
                        ...
                        if(malformed[0] > 0)
                                StargateJourney.LOGGER.warn("TransporterNetwork.Dimension: skipped {} transporter(s) with malformed UUID keys in dimension {}", malformed[0], dimensionTag.getString(DIMENSION));
